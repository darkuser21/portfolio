...
app.use(express.static(__dirname)); // Serve static files from the repository root
...
app.post("/send-inquiry", async (req, res) => {
  let { name, number, email, service, message } = req.body;

  // Make service optional by providing a fallback
  if (!service) {
  ...
  if (!name || !number || !email) {
    return res
      .status(400)
      .json({ error: "Missing required fields (Name, Number, or Email)" });
  }
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="data-box">
              <p><span class="field-label">Service Required:</span> ${service}</p>
  ...
  const mailToUser = {
    ...
    to: email,
    subject: `Confirmation: Inquiry Received regarding ${service}`,
    html: `
      <html>
      <head>${cyberStyle}</head>
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="data-box" style="border-left-color: #2563eb;">
              <p><span class="field-label">Requested Service:</span> ${service}</p>
            </div>
...
app.listen(port, () => {
  console.log(`[+] Secure Node listener active on port ${port}...`);
});
